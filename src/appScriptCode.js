...
const sheetName = getSheetsName(activePage.sheetId || listSheetInformation.Validasi1.sheetId)
const index = (targetSheetName) => sheetName.findIndex(name => targetSheetName.test(name))
const sheet = ss(activePage.sheetId || listSheetInformation.Validasi1.sheetId).getSheetByName(sheetName[index(sheetEnumList)]);


function doGet(e) {
  let page = e.parameter.mode || "validasi-1";
  let html = HtmlService.createTemplateFromFile(`src/page/${page}`).evaluate().setTitle("Tools QC KSE");
  let htmlOutput = HtmlService.createHtmlOutput(html);

  userProperties.setProperty('mode', page);

  // add metatag to html
  htmlOutput.addMetaTag('viewport', 'width=device-width, initial-scale=1, maximum-scale=1.0, user-scalable=0');
          ...
          <ul class="navbar-nav justify-content-end flex-grow-1 pe-3" id="nav-links">
            ${navLinks.map(link => `
              <li class="nav-item" data-bs-dismiss="offcanvas">
                <a class="nav-item nav-link ${link.isActive ? 'active' : ''}" href="${link.url}">${link.text}</a>
              </li>
            `).join('')}
          </ul>
        </div>
...
function uploadData(formData) {
  ...
  const sheetAssessmentKolam = ss(activePage.sheetId).getSheetByName(activePage.sheetUploadData);
  const sheetDokumentasiAssessment = ss(activePage.sheetId).getSheetByName(activePage.sheetUploadImage);
  const lastRowSheetDokumentasi = sheetDokumentasiAssessment.getRange('B1:B').getValues().filter(String).length;
  ...
  const pondId = `${leadId}${'0'.repeat(4 - pondNumber?.toString().length)}${pondNumber}`;
  ...
  sheetAssessmentKolam.getRange(lastRowSheetAssessment + 1, 1, 1, dataForSheet[0].length).setValues(dataForSheet);
  sheetDokumentasiAssessment.getRange(lastRowSheetDokumentasi + 1, 1).setValue(assessmentId);

  if(activePage.orderedKeyUploadData.some(key => /^setFormula/.test(key))){
    const setFormula = activePage.orderedKeyUploadData.filter(key => /^setFormula/.test(key))

    setFormula.forEach((key, index) => {
      activePage[key](sheetAssessmentKolam, lastRowSheetAssessment + 1, lastColumnSheetAssessment + index)
    })
  }
...
function uploadImages(obj) {
  ...
  const sheetDokumentasiValidasi = ss(activePage.sheetId).getSheetByName(activePage.sheetUploadImage);
  const lastActiveRow = sheetDokumentasiValidasi.getRange('B1:B').getValues().filter(String).length;
  const folder = rootFolder.createFolder(documentationId);

  sheetDokumentasiValidasi.getRange(lastActiveRow + 1, 2).setValue(documentationId);
  sheetDokumentasiValidasi.getRange(lastActiveRow + 1, 3).setValue(userEmail);
  
  obj.forEach((e, index) => {
    const blob = Utilities.newBlob(Utilities.base64Decode(e.data), e.mimeType, e.fileName);
    const url = folder.createFile(blob).getUrl();
    const lastActiveCol = sheetDokumentasiValidasi.getRange(lastActiveRow + 1, 2, 1, 10).getValues()[0].filter(String).length;
    sheetDokumentasiValidasi.getRange(lastActiveRow + 1, lastActiveCol + 2).setValue(url || "-");
  });
