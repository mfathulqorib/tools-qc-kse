const ss = (sheetIdFromFunction) => SpreadsheetApp.openById(sheetIdFromFunction) 
const timeStamp = Utilities.formatDate(new Date(), 'GMT+7', 'yyyy-MM-dd\' \'HH:mm:ss');
const userEmail = Session.getActiveUser().getEmail();
const getSheetsName = (sheetId) => {
  const data = ss(sheetId).getSheets().map((sheet) => sheet.getName()).sort((a, b) => {
...
const listSheetInformation = {
  'validasi-1': {
    ...
    orderedKeyUploadData: [
      'nama', 'lead_id', 'area', 'komoditas', 'nomor_kolam', 'jenis_kolam', 'suhu',
      'dissolved_oxygen', 'ph', 'tds', 'lokasi_kolam', 'luas', 'kedalaman', 'sumber_air',
      'debit_air', 'padat_tebar', 'hystorical_pond_cycle', 'hystorical_pond_commodity',
      'numeric_historikal_tebaran', 'satuan_tebaran', 'numeric_historikal_panen',
      'historikal_doc', 'farmer_status', 'apakah_kolam_kosong', 'apakah_koperatif',
      'apakah_ada_kandang_hewan', 'apakah_dekat_jurang', 'apakah_dekat_jalan',
      'apakah_dekat_pemukiman', 'apakah_banyak_predator', 'apakah_ada_listrik',
      'apakah_pasang_feeder', 'pond_id', 'sumber_benih'
    ],
    columnRanges: {
      area: 'A2:A',
      'list-petani': 'B2:C',
  ...
  'validasi-midway': {
    ...
    orderedKeyUploadData: [
      'nama', 'lead_id', 'area', 'nomor_kolam', 'komoditas', 'tanggal_tebar',
      'numeric_tebaran', 'satuan_tebaran', 'estimasi_siap_panen', 'satuan_waktu',
      'numeric_estimasi_panen', 'apakah_sedang_wabah', 'apakah_ikan_kuntet',
      'apakah_ada_buyer', 'numeric_harga_buyer', 'kontak_buyer', 'pond_id',
      'apakah_bisa_sampling', 'bobot_sampling_ikan'
    ],
    columnRanges: {
      'area': 'A2:A',
      'list-petani': 'B2:C',
  ...
  'validasi-2': {
    ...
    orderedKeyUploadData: [
      'nama', 'lead_id', 'area', 'nomor_kolam', 'status_kesiapan_kolam',
      'status_kesiapan_kolam_lainnya', 'estimasi_siap_tebar', 'satuan_waktu',
